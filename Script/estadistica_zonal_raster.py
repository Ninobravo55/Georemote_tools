...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        v_layer = self.parameterAsVectorLayer(
            parameters, self.INPUT_VECTOR, context)
        r_layer = self.parameterAsRasterLayer(
        ...
        if v_layer is None:
            raise QgsProcessingException(
                "No se pudo cargar la capa de poligonos.")
        if r_layer is None:
        ...
        feedback.pushInfo(
            f"Vector  : {
                v_layer.name()} ({
                v_layer.featureCount()} poligonos)")
        ...
        if band_n < 1 or band_n > ds_r.RasterCount:
            raise QgsProcessingException(
                f"Banda {band_n} fuera de rango (raster tiene {
                    ds_r.RasterCount})."
            )
        ...
        if srs_r.GetAuthorityCode(None):
            crs_r_authid = f"{
                srs_r.GetAuthorityName(None)}:{
                srs_r.GetAuthorityCode(None)}"
        ...
        for i, feat in enumerate(feats):
            ...
            if inv_gt is None:
                raise QgsProcessingException(
                    "GeoTransform invalido del raster.")
            px_min, py_min = gdal.ApplyGeoTransform(
                inv_gt, bbox.xMinimum(), bbox.yMaximum())
            px_max, py_max = gdal.ApplyGeoTransform(
                inv_gt, bbox.xMaximum(), bbox.yMinimum())
        ...
        if not registros:
            raise QgsProcessingException(
                "No se generaron registros (revisa CRS y extension).")
