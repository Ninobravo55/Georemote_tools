...
from qgis.PyQt.QtWidgets import (
    ...
    QMessageBox, QGroupBox, QDialogButtonBox, QApplication
)
...
def _construir_capas(filas, campo_grupo, campo_orden, campo_x, campo_y, crs):
    ...
    for g, lista in grupos.items():
        for orden, x, y in lista:
            ...
            f.setAttributes(
                [str(g), int(orden), f"V{int(orden)}", float(x), float(y)])
...
class CrearPoligonosTablaDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        # ---- carpeta salida ----
        gb_out = QGroupBox(
            "4. Carpeta de salida (Shapefiles vertices/segmentos/poligonos)")
    ...
    def _cargar_columnas(self):
        ...
        if not ruta:
            QMessageBox.warning(
                self, "Aviso", "Selecciona primero un archivo.")
            return
        try:
        ...
        _set_default(
            self.cb_x, [
                "este_x", "este", "x", "longitud", "lon", "easting"])
        _set_default(
            self.cb_y, [
                "norte_y", "norte", "y", "latitud", "lat", "northing"])
...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        sep = self.parameterAsString(
            parameters, self.SEPARATOR, context) or ","
        ...
        feedback.pushInfo(
            f"Campos  : grupo={f_grp}  orden={f_ord}  x={f_x}  y={f_y}")
    ...
    def run(self):
        ...
        if not v["archivo"]:
            QMessageBox.warning(
                parent, "Aviso", "Debes seleccionar una tabla.")
            return
        if not v["carpeta"]:
            QMessageBox.warning(
                parent, "Aviso", "Debes seleccionar la carpeta de salida.")
            return
        for k in ("campo_grupo", "campo_orden", "campo_x", "campo_y"):
            if not v[k]:
                QMessageBox.warning(
                    parent, "Aviso", f"Selecciona el campo '{k}'.")
                return
        ...
        if v["cargar"]:
            try:
                ...
                QgsMessageLog.logMessage(
                    f"No se pudieron cargar las capas generadas en el "
                    f"proyecto: {e}",
