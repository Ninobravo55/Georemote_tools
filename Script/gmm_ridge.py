    ...
    def check_is_fitted(estimator, attributes):
        ...
        for attr in attributes:
            if not hasattr(estimator, attr):
                raise RuntimeError(
                    f"Model not fitted. Missing attribute: {attr}")
...
def _predict_worker(tau: float, model: "GMMR", xT: np.ndarray,
                    yT: np.ndarray) -> np.ndarray:
...
class GMMR(BaseEstimator, ClassifierMixin):
    ...
    def learn(self, x: np.ndarray, y: np.ndarray) -> "GMMR":
        ...
        """
        # Normalize labels to 1D to avoid sklearn DataConversionWarning on
        # column vectors.
        ...
        if not np.all(np.isfinite(unique_labels)):
            raise ValueError(
                "Non-finite class labels detected in training data")
        ...
        for c, cR in enumerate(unique_labels):
            ...
            if self.reg_type != "ridge":
                self.cov[c, :, :] = self._apply_advanced_regularization(
                    empirical_cov, j.size)
            else:
    ...
    def predict(
        ...
        if xt.shape[1] != self.n_features_in_:
            raise ValueError(
                f"X has {
                    xt.shape[1]} features but model was trained on {
                    self.n_features_in_} features")
        ...
        for c in range(C):
            ...
            # Compute inverse via eigendecomposition: Sigma^{-1} = Q *
            # diag(1/L) * Q^T
    ...
    def predict_proba(self, X: np.ndarray,
        ...
        if X.shape[1] != self.n_features_in_:
            raise ValueError(
                f"X has {
                    X.shape[1]} features but model was trained on {
                    self.n_features_in_} features")
    ...
    def compute_inverse_logdet(
            self, c: int, tau: float) -> Tuple[np.ndarray, float]:
    ...
    def BIC(self, x: np.ndarray, y: np.ndarray,
    ...
    def _cross_validation_legacy(
        ...
        for label in unique_labels:
            ...
            for fold_idx in range(v):
                start = fold_idx * fold_size
                    ...
                    fold_size if fold_idx < v - 1 else n_samples
        ...
        for i in range(v):
            ...
            train_idx = np.concatenate([fold_indices[j]
                                       for j in range(v) if j != i])
        ...
        if n_jobs != 1:
            try:
                ...
                for i in range(v):
                    ...
                    processes.append(
                        pool.apply_async(
                            _predict_worker, args=(
                                tau, model_cv[i], x[test_idx], y[test_idx])),
                    )
    ...
    def get_feature_importance(self, method: str = "variance") -> np.ndarray:
        ...
        if method == "variance":
            ...
            overall_mean = np.average(
                self.mean, axis=0, weights=self.prop.ravel())
            ...
            raise ValueError(
                f"Unknown method: {method}. Use 'variance' or 'discriminative'")
    ...
    def partial_fit(self, X: np.ndarray, y: np.ndarray,
                    classes: Optional[np.ndarray] = None) -> "GMMR":
        ...
        if not hasattr(self, "mean"):
            if classes is None:
                raise ValueError(
                    "classes must be provided on first call to partial_fit")
        ...
        if X.shape[1] != self.n_features_in_:
            raise ValueError(
                f"X has {
                    X.shape[1]} features but model expects {
                    self.n_features_in_} features")
    ...
    def _apply_advanced_regularization(
            self, cov: np.ndarray, n_samples: int) -> np.ndarray:
        ...
        if self.reg_type == "ledoit_wolf":
            ...
            if self.shrinkage_target == "diagonal":
                ...
                raise ValueError(
                    f"Unknown shrinkage_target: {
                        self.shrinkage_target}")
        ...
        raise ValueError(
            f"Unknown reg_type: {
                self.reg_type}. Use 'ridge', 'ledoit_wolf', 'oas', or 'empirical'")
    ...
    def _ledoit_wolf_shrinkage(self, cov: np.ndarray, n_samples: int) -> float:
        ...
        mu = np.trace(
            cov) / d if self.shrinkage_target == "identity" else np.mean(np.diag(cov))
    ...
    def _oas_shrinkage(self, cov: np.ndarray, n_samples: int) -> float:
        ...
        # OAS formula
        rho = min(((1 - 2 / d) * trace_cov2 + trace_cov**2) /
                  ((n + 1 - 2 / d) * (trace_cov2 - trace_cov**2 / d)), 1.0)
    ...
    def __setstate__(self, state: Dict[str, Any]) -> None:
        ...
        self.reg_type = state.get(
            "reg_type", "ridge")  # Backward compatibility
