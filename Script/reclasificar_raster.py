...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        keep_unmapped = self.parameterAsBool(
            parameters, self.KEEP_UNMAPPED, context)
        ...
        if len(flat) == 0:
            raise QgsProcessingException(
                "La tabla de remapeo esta vacia. Agregue al menos una fila.")
        if len(flat) % 2 != 0:
            raise QgsProcessingException(
        ...
        for i in range(0, len(flat), 2):
            ...
            try:
                ...
                raise QgsProcessingException(
                    "Fila {}: valores no numericos -> {}".format(
                        i // 2 + 1, flat[i:i + 2])
                )
            if v_old in seen_old:
                raise QgsProcessingException(
                    "Fila {}: el valor {} aparece mas de una vez en 'Valor anterior'. "
                    "Cada valor debe mapearse exactamente una vez.".format(
                        i // 2 + 1, v_old)
                )
            seen_old.add(v_old)
        ...
        feedback.pushInfo(
            "Raster entrada      : {}".format(
                os.path.basename(input_path)))
        feedback.pushInfo("Banda               : {}".format(band_num))
        feedback.pushInfo("Reglas de remapeo   : {}".format(len(rules)))
        feedback.pushInfo(
            "Conservar no mapeados: {}".format(
                "Si" if keep_unmapped else "No -> NoData"))
        feedback.pushInfo("-" * 50)
        for r in rules:
            if 'feedback' in locals() and feedback.isCanceled():
                break
            feedback.pushInfo(
                "  {:>8.2f}  ->  {:>8.2f}".format(r["old"], r["new"]))
        feedback.pushInfo("=" * 50)
