    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if raster_layer is None:
            raise QgsProcessingException(
                "No se pudo leer el raster clasificado.")

        input_path = raster_layer.source()
        band_num = self.parameterAsInt(parameters, self.BAND, context)
        legend_flat = self.parameterAsMatrix(
            parameters, self.LEGEND_TABLE, context)
        ...
        if ds is None:
            raise QgsProcessingException(
                "GDAL no pudo abrir el raster clasificado.")
        ...
        if band_num < 1 or band_num > nbands:
            raise QgsProcessingException(
                "Banda {} no existe (el raster tiene {} banda(s)).".format(
                    band_num, nbands)
            )
        ...
        for i, cls in enumerate(clases_unicas):
            ...
            pct_valido = (
                n_pix /
                n_px_validos *
                100) if n_px_validos > 0 else 0.0
        ...
        # fila donde arranca la tabla (2 + 8 meta + 1 espacio = 11)
        ROW_START = len(meta) + 3
        ...
        feedback.pushInfo(
            "Area valida     : {:,.4f} ha".format(area_valida_ha))
        feedback.pushInfo("Px NoData       : {:,}".format(n_px_nodata))
