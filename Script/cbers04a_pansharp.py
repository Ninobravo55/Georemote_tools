...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        aoi_layer = self.parameterAsVectorLayer(
            parameters, self.INPUT_AOI, context)
        output_ms = self.parameterAsOutputLayer(
            parameters, self.OUTPUT_MS, context)
        output_ps = self.parameterAsOutputLayer(
            parameters, self.OUTPUT_PS, context)
        ...
        archivos = [f for f in os.listdir(
            folder) if f.lower().endswith(".tif")]
        ...
        if bandas_faltantes:
            raise Exception(
                f"❌ Faltan bandas multiespectrales: {bandas_faltantes}")
        ...
        # Exportar a shapefile temporal (gdal.Warp Python solo acepta ruta, no
        # objeto en memoria)
        ...
        if result is None or not os.path.exists(pan_clip_path):
            raise Exception(
                "❌ Error al recortar PAN. Verifica que el AOI se solape con la imagen.")
        ...
        for idx, ms_path in enumerate(ms_paths):
            ...
            ms_resampled[idx] = ds_ms.GetRasterBand(
        ...
        ds_out_ms = None
        feedback.pushInfo(
            f"   MS exportado: {cols_ms} x {rows_ms} px @ {ms_res_nat:.1f} m")
