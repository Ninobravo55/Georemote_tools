...
def _adivinar_nombres(path, nbands):
    ...
    if nbands == 6 and any(t in p for t in (
            "lt05", "lt04", "le07", "lt5", "le7")):
...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        bands_str = self.parameterAsString(
            parameters, self.BANDS, context).strip()
        ...
        if bands_str:
            ...
            for b in bandas:
                if b < 1 or b > info["nbands"]:
                    raise QgsProcessingException(
                        f"Banda {b} fuera de rango (la imagen tiene {
                            info['nbands']})."
                    )
        else:
        ...
        with open(info_txt, "w", encoding="utf-8") as fh:
            ...
            for i, nm in enumerate(info["band_names"], 1):
                fh.write(
                    f"  Banda {i:2d}: {nm}  (NoData={info['nodata'][i - 1]})\n")
        ...
        feedback.pushInfo(
            f"OK - {len(salidas)} bandas extraidas en: {out_dir}")
