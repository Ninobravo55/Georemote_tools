...
"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        output_acp = self.parameterAsOutputLayer(
            parameters, self.OUTPUT_ACP, context)
        output_bar = self.parameterAsFileOutput(
            parameters, self.OUTPUT_BARRA, context)
        output_corr = self.parameterAsFileOutput(
            parameters, self.OUTPUT_CORR_CSV, context)
        output_cpng = self.parameterAsFileOutput(
            parameters, self.OUTPUT_CORR_PNG, context)
        ...
        if output_cpng:
            try:
                ...
                ax.set_title(
                    "Matriz de correlación entre bandas",
                    fontsize=14,
                    pad=15)
                ...
                feedback.pushInfo(
                    f"   ⚠ No se pudo generar PNG correlación: {e}")
        ...
        feedback.pushInfo(
            f"   ➤ PCA1+PCA2+PCA3 = {var_total_3:.2f} % varianza")
