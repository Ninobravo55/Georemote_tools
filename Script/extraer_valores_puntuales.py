...
from qgis.PyQt.QtWidgets import (
    ...
    QTreeWidgetItem, QCheckBox, QAbstractItemView, QProgressBar
)
...
from qgis.core import (
    ...
    QgsFeature, QgsField, QgsFields, QgsVectorFileWriter, QgsWkbTypes,
    QgsRaster, QgsMessageLog, Qgis
)
...
def _muestrear_nearest(rlayer, point_in_raster_crs, banda):
    ...
    res = dp.identify(point_in_raster_crs, QgsRaster.IdentifyFormat.IdentifyFormatValue)
    if not res.isValid():
        return None
    vals = res.results()
...
class ExtraerValoresPuntualesDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        # ---- 2. Rasters / bandas ----
        gb_r = QGroupBox(
            "2. Rasters disponibles (marca los que se muestrearan, banda por banda)")
        v_r = QVBoxLayout(gb_r)
        self.tree = QTreeWidget()
        self.tree.setHeaderLabels(
            ["Raster / Banda", "Nombre del campo de salida"])
        ...
        self.chk_xy = QCheckBox(
            "Incluir coordenadas X / Y del punto en la salida")
        self.chk_xy.setChecked(True)
        f_o.addRow(self.chk_xy)

        self.chk_copy_attr = QCheckBox(
            "Copiar atributos de la capa de puntos al resultado")
        ...
        self.chk_load = QCheckBox(
            "Agregar resultado al proyecto (solo para GPKG/SHP)")
        ...
        btns.button(QDialogButtonBox.StandardButton.Ok).setText("Ejecutar")
        ...
        for lyr_id, lyr in proyecto.mapLayers().items():
            if isinstance(lyr, QgsVectorLayer) and lyr.geometryType(
                ...
                for b in range(1, lyr.bandCount() + 1):
                    ...
                    child.setFlags(
                        child.flags() | Qt.ItemFlag.ItemIsUserCheckable | Qt.ItemFlag.ItemIsEditable)
                    child.setCheckState(0, Qt.CheckState.Unchecked)
                    child.setData(0, Qt.ItemDataRole.UserRole, b)  # numero de banda
    ...
    def _toggle_all(self, marcar):
        st = Qt.CheckState.Checked if marcar else Qt.CheckState.Unchecked
    ...
    def get_values(self):
        ...
        for i in range(self.tree.topLevelItemCount()):
            top = self.tree.topLevelItem(i)
            rid = top.data(0, Qt.ItemDataRole.UserRole)
            for j in range(top.childCount()):
                child = top.child(j)
                if child.checkState(0) == Qt.CheckState.Checked:
                    banda = child.data(0, Qt.ItemDataRole.UserRole)
                    rasters.setdefault(rid, []).append(banda)
                    nombres[(rid, banda)] = child.text(
                        1).strip() or f"r_{rid}_b{banda}"
        return {
...
def _ejecutar_extraccion(point_layer, raster_specs, interp_idx,
    ...
    for i, feat in enumerate(point_layer.getFeatures()):
        ...
        try:
            ...
            log(f"  Aviso: se omite feature {feat.id()} sin punto valido: {e}")
            continue
...
"""
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterMultipleLayers(
            self.INPUT_RASTERS, self.tr(
                "Rasters a muestrear (todas las bandas)"),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if src is None:
            raise QgsProcessingException(
                "No se pudo cargar la capa de puntos.")
        rasters = self.parameterAsLayerList(
            parameters, self.INPUT_RASTERS, context)
        if not rasters:
        ...
        for r in rasters:
            ...
            nombres = {b: (f"{r.name()}_b{b}" if r.bandCount() > 1 else r.name())
                       for b in bandas}
            raster_specs.append(
                {"layer": r, "bandas": bandas, "nombres": nombres})
        ...
        feedback.pushInfo(
            f"Interpolacion: {
                'Bilineal' if interp == 1 else 'Nearest'}")
        feedback.pushInfo(
            "====================================================")
    ...
    def run(self):
        ...
        if not v["puntos_id"]:
            QMessageBox.warning(
                parent, "Aviso", "Selecciona la capa de puntos.")
            return
        if not v["rasters"]:
            QMessageBox.warning(
                parent, "Aviso", "Marca al menos una banda raster.")
            return
        if not v["salida"]:
            QMessageBox.warning(
                parent, "Aviso", "Define el archivo de salida.")
            return
        ...
        if v["cargar"] and ext in (".gpkg", ".shp"):
            try:
                ...
                QgsMessageLog.logMessage(
                    f"No se pudo cargar '{ruta}' en el proyecto: {e}",
